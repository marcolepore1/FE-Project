function [eta, k, sigma, MSE, MAPE] = calibrateATSNIGput(setDate, optionTable, Forwards, discountCurve)
%
% Function that calibrates time dependent parameters of ATSNIG from data
% provided in optionTable using only OTM put options
%
% INPUT
...
OTMstrikes               = options.STRIKES(idx(2):2:idx(end)).*(Forwards(1) > options.STRIKES(idx(2):2:idx(end)));
OTMstrikes               = OTMstrikes(find(OTMstrikes));
[sigma(1), eta(1), k(1), MSE(1), MAPE(1)] = CalibrateNMVMToVolatilitySurface(setDate, discounts(1), Forwards(1), OTMstrikes, [],...
    OTMputs, maturities(1), alpha, x00, 1);
...
for i=2:length(maturities)
    ...
    OTMputs         = OTMputs(find(OTMputs));
    OTMstrikes      = options.STRIKES(idx(2):2:idx(end)).*(Forwards(i) > options.STRIKES(idx(2):2:idx(end)));
    OTMstrikes      = OTMstrikes(find(OTMstrikes));
    x00             = [sigma(i-1), eta(i-1), k(i-1)]';
    [sigma(i), eta(i), k(i), MSE(i), MAPE(i)] = CalibrateNMVMToVolatilitySurfaceConst(setDate, discounts(i), Forwards(i), OTMstrikes, [],...
        OTMputs, maturities(i), TTM(i-1), alpha, x00, 1);
%     [sigma(i), eta(i), k(i)] = CalibrateNMVMToVolatilitySurface(setDate, discounts(i), Forwards(i), OTMstrikes,...
%         OTMputs, maturities(i), alpha, x00);
end
