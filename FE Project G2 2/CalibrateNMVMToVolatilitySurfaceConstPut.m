function [sigma, eta, k, MSE, MAPE] = CalibrateNMVMToVolatilitySurfaceConstPut(setDate, discount, Forward, strikes, prices, maturity, prevTTM, alpha, x00)
%
...
% constraints of NIG & increasing g1, g2, & g3
...
% aux   = @(cal) sum(abs(MktPrices-MdlPrices(cal)').^2); % function that we want to minimize the weigths are negligible since they are equal to 1

% constraints of the calibration
% x00 = [0.3; 1; TTM]; % sigma, eta, k
LB = [0.001; -5; 0.001];
UB = [1; 50; 5];
